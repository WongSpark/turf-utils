...
class TurfUtil{
     ...
     * @param options
     */
    static rhumbAlong(line:LineString,distance:number,options: {units?: Units} = {}) : Feature<Point>{
     ...
     * @returns {number}
     */
    static rhumbAlongBearing(line:LineString,distance:number,options: {units?: Units} = {}) : number{
        const geom = getGeom(line);
        const coords = geom.coordinates;
        let travelled = 0;
        for (let i = 0; i < coords.length; i++) {
